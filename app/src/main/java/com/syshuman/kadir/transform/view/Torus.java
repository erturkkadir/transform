...
import com.syshuman.kadir.transform.R;
...
/**
 * Created by kerturkx on 2017-10-27.
 */
...
public class Torus {
    ...
    public Torus(Context context, float[] data_x, float[] data_y, float[] data_z, float[] faces) {

        length = data_x.length;
        // Create buffer for vertices
        ByteBuffer buffer1 = ByteBuffer.allocateDirect(data_x.length * 3 * 4);
        buffer1.order(ByteOrder.nativeOrder());
        verticesBuffer = buffer1.asFloatBuffer();

        // Create buffer for faces
        ByteBuffer buffer2 = ByteBuffer.allocateDirect(faces.length * 3 * 2);
        ...
        for (int i = 0; i < data_x.length; i++) {
            facesBuffer.put((short) (data_x[i] - 1));
            facesBuffer.put((short) (data_y[i] - 1));
            facesBuffer.put((short) (data_z[i] - 1));
        }
    ...
    public void draw() {
        ...
        GLES20.glDrawElements(GLES20.GL_TRIANGLES, length * 3, GLES20.GL_UNSIGNED_SHORT, facesBuffer);
